 ...
 * ints and longs changed to int64 or uint64
 ...
 * pointers changed to uint64 when their values are important
...
#define ROWNO 21
struct rm {
    int64 glyph;            /* what the hero thinks is there */
...
typedef struct fe {
    ...
    int64 timeout;          /* when we time out */
    uint64 tid;             /* timer ID */
...
struct monst {
    struct monst *nmon;
    uint64 data;            /* officially (struct permonst *) */
    uint64 m_id;
    ...
    int64 mhp, mhpmax;
    uint64 mappearance;     /* for undetected mimics and the wiz */
    ...
    int64 mspec_used;       /* monster's special ability attack timeout */
    ...
    int64 mstrategy;        /* for monsters with mflag3: current strategy */
    int64 mtrapseen;        /* bitmap of traps we've been trapped in */
    int64 mlstmv;           /* for catching up with lost time */
    int64 mgold;            /* ifndef GOLDOBJ */
    struct obj *minvent;
    struct obj *mw;
    int64 misc_worn_check;
    int8 weapon_check;
    uint8 mnamelth;         /* length of name (following mxlth) */
    int16 mxlth;            /* length of following data */
    int64 meating;          /* monster is eating timeout */
    int64 mextra[1];        /* monster dependent info */
...
struct egd {
    int64 fcbeg, fcend;     /* fcend: first unused pos */
    int64 vroom;            /* room number of the vault */
...
struct bill_x {
    uint64 bo_id;
    int8 useup;
    int64 price;            /* price per unit */
    int64 bquan;            /* amount used up */
};

#define BILLSZ 200
#define PL_NSIZ 32          /* name of player, ghost, shopkeeper */
struct eshk {
    int64 robbed;           /* amount stolen by most recent customer */
    int64 credit;           /* amount credited to customer */
    int64 debit;            /* amount of debt for using unpaid items */
    int64 loan;             /* shop-gold picked (part of debit) */
    int64 shoptype;         /* the value of rooms[shoproom].rtype */
    ...
    int64 billct;           /* no. of entries of bill[] in use */
    struct bill_x bill[BILLSZ];
    struct bill_x *bill_p;
    int64 visitct;          /* nr of visits by most recent customer */
...
struct edog {
    int64 droptime;         /* moment dog dropped object */
    uint64 dropdist;        /* dist of drpped obj from @ */
    int64 apport;           /* amount of training */
    int64 whistletime;      /* last time he whistled */
    int64 hungrytime;       /* will get hungry at this time */
    coord ogoal;            /* previous goal location */
    int64 abuse;            /* track abuses to this pet */
    int64 revivals;         /* count pet deaths */
    int64 mhpmax_penalty;   /* while starving, points reduced */
...
struct obj {
    ...
    int64 corpsenm;         /* type of corpse is mons[corpsenm] */
    uint64 oeaten;          /* nutrition left in food, if partly eaten */
    int64 age;              /* creation date */
...
struct engr {
    ...
    char *engr_txt;
    int8 engr_x, engr_y;
    uint64 engr_lth;        /* for save & restore; not length of text */
    int64 engr_time;        /* moment engraving was (will be) finished */
    int8 engr_type;
};
...
typedef struct {
    ...
    uint64 attach_2_m;      /* Region attached to monster ? */
    ...
    int8 player_flags;
    uint64 *monsters;       /* Monsters currently inside this region */
    ...
    int64 glyph;            /* Which glyph to use if visible */
    uint64 arg;             /* Optional user argument */
} NhRegion;
